...
filters_name = ['Gabor' + str(i).zfill(2) for i in np.arange(0, 56, 1) + 1]
...
classified_img_folder = '/mnt/CEPH_PROJECTS/ALPSNOW/Riccardo/HYPERSPECTRAL/Downsampled_UAV/00_classified_Gabor_features_xgboost_56featR7'

# Example usage:
image_paths = sorted(glob.glob(os.path.join(classified_img_folder, '*snow_map')))

output_path = os.path.join('/mnt/CEPH_PROJECTS/ALPSNOW/Riccardo/HYPERSPECTRAL/Downsampled_UAV', '_'.join(image_paths[0].split('_')[-4:]) + '_56feat.tif')
